...
existing product. It is varied on purpose — a missing material here, a single
colour there, a feminine category, an accented name — because a template is
only worth what it does with an incomplete record.
"""

import re
from collections import Counter

from n0 import describe
...
def test_renders_a_complete_product():
...
def test_agrees_with_a_feminine_category():
...
def test_a_single_item_is_not_enumerated():
    described = describe(CATALOGUE[2])
    assert "À choisir en bleu nuit." in described
    assert " et " not in described


def test_prefers_the_wording_that_uses_the_most_attributes():
...
def test_skips_the_blocks_whose_attributes_are_missing():
...
def test_falls_back_to_the_shortest_wording_when_only_the_name_remains():
    assert describe({"name": "Ténor", "category": "réveil", "gender": "m"}) == "Ténor : un réveil."


def test_ignores_empty_values_and_empty_list_items():
    described = describe(
        {
            "name": "Volute",
            "category": "vase",
...
def test_an_empty_record_renders_nothing_rather_than_a_broken_sentence():
    assert describe({}) == ""


def test_the_same_product_always_gets_the_same_description():
    assert describe(CATALOGUE[4]) == describe(CATALOGUE[4])
...
def frame(product: dict) -> str:
    ...
    for key, value in product.items():
        if key == "gender":  # grammar, not a value the reader sees
            continue
...
def test_breaking_point_the_catalogue_is_written_from_a_handful_of_frames():
    """
    ...
    wordings to a complete record, but one product draws all four from the same
    seed: the ceiling is the least common multiple of those, six, doubled by
    the two grammatical genders of the fixture. Twelve frames for two hundred
    products.
    ...
    assert min(frames.values()) >= len(full) // 16


def test_breaking_point_holds_on_records_that_differ_in_shape():
    """
    ...
    assert len(openings) <= 9 < len(CATALOGUE)
    assert openings.most_common(1)[0][1] >= len(CATALOGUE) // 4
