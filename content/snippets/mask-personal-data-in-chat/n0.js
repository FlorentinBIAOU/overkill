 ...
 *
 * Second, the phone pattern tolerates the separators people type between
...
const IBAN = /(?<![A-Z0-9])[A-Z]{2} ?\d{2}(?: ?[A-Z0-9]{4}){2,7}(?: ?[A-Z0-9]{1,3})?(?![A-Z0-9])/gi;

// Order matters: an email may contain digits that would otherwise be read as
// the start of a phone number.
const PATTERNS = [[EMAIL, '[email]'], [IBAN, '[iban]'], [PHONE, '[phone]']];
