...
the one that is due, so that the sum EN 16931 makes a rule of — total with VAT
= total without VAT + VAT — can be recomputed on what the model wrote. It
catches nothing the model gets consistently wrong, and it catches a digit read
off the wrong line, which is the failure this entry is about.
"""
...
FIELDS = ("invoice_number", "date", "total_excluding_vat", "vat", "total")
AMOUNTS = ("total_excluding_vat", "vat", "total")
...
def _decode(answer: str) -> dict:
    """
    Turn the answer into the five fields, or refuse it.
    ...
    for field in AMOUNTS:
        ...
        if value is not None and (isinstance(value, bool) or not isinstance(value, (int, float))
            ...
            raise ExtractionUnavailable(f"the model answered a {field} that is not a number: {value!r}")
    return {**fields, "totals_agree": _totals_agree(fields)}


def _totals_agree(fields: dict) -> bool | None:
    """
    BR-CO-15 recomputed on what the model wrote, or None if it wrote too little.
    ...
    """
    amounts = [fields[field] for field in AMOUNTS]
