...
def test_point_de_rupture_2024_02_31_en_json_impeccable_est_ecarte_et_la_date_inventee_passe():
    ...
    assert extract_dates("31 février 2024, hier", client=client, reference=TODAY) == [
        ("hier", date(2024, 3, 11))
    ]
...
def test_point_de_rupture_rien_dans_la_reponse_ne_signale_une_date_absente_du_document():
    ...
    assert extract_dates(document, client=client, reference=TODAY) == [("15 mars", date(2024, 3, 15))]
...
def test_point_de_rupture_une_reponse_en_prose_leve_plutot_que_rendre_une_liste_vide():
    ...
    with pytest.raises(ExtractionUnavailable):
        extract_dates("réunion le 12/03/2024", client=prose, reference=TODAY)
    assert prose.call_count == 3
    assert extract_dates("rien à signaler", client=FakeLLM(response="[]"), reference=TODAY) == []
...
def test_point_de_rupture_une_cle_mal_nommee_ou_une_date_ecrite_12_03_2024_est_redemandee_puis_leve():
    ...
    for response in (
        ...
        with pytest.raises(ExtractionUnavailable, match="list of ISO days"):
            extract_dates("réunion le 12/03/2024", client=client, reference=TODAY)
...
def test_point_de_rupture_rien_dans_la_requete_n_empeche_la_reponse_de_porter_2024_02_31():
    ...
    sdk = FakeSDK(content="[]")
    extract_dates("réunion le 12/03/2024", client=ProviderClient(sdk=sdk), reference=TODAY)
...
def test_decode_ce_que_le_modele_annonce():
    ...
    assert extract_dates("réunion le 12/03/2024", client=client, reference=TODAY) == [
...
def test_une_date_relative_resolue_par_le_modele_est_rendue():
    ...
    assert extract_dates("on se voit jeudi prochain", client=client, reference=TODAY) == [
...
def test_envoie_le_texte_le_jour_de_reference_et_la_consigne_a_temperature_zero():
    ...
    extract_dates("on se voit jeudi prochain", client=client, reference=TODAY)
    prompt = client.last_request["prompt"]
    assert prompt == PROMPT.format(text="on se voit jeudi prochain", reference="2024-03-12")
    assert "Resolve relative dates" in prompt and "document, which is 2024-03-12" in prompt
...
def test_la_reference_est_obligatoire_et_c_est_la_date_du_document():
    """
    Docstring : « The reference is the date of the document, not the day of the
    run […] It has no default here, on purpose, because a default would quietly
    be today ».
    """
    client = FakeLLM(response="[]")
    ...
    with pytest.raises(TypeError, match="date of the document"):
        extract_dates("on se voit jeudi prochain", client=client, reference=None)
    with pytest.raises(TypeError, match="date of the document"):
        extract_dates("on se voit jeudi prochain", client=client, reference="2024-03-12")
    assert client.call_count == 0
    # La référence passée est celle qui part dans la requête, et rien d'autre.
    extract_dates("on se voit jeudi prochain", client=client, reference=date(2023, 11, 2))
    assert "document, which is 2023-11-02" in client.last_request["prompt"]
...
def test_le_document_entier_part_pas_les_seules_dates():
    ...
    client = FakeLLM(response="[]")
    extract_dates(document, client=client, reference=TODAY)
...
def test_refuse_une_entree_trop_grande_avant_de_depenser_quoi_que_ce_soit():
    ...
    with pytest.raises(ValueError):
        extract_dates("x" * (MAX_CHARACTERS + 1), client=client, reference=TODAY)
    assert client.call_count == 0


def test_une_panne_est_retentee_et_reussit_au_troisieme_essai():
    client = FakeLLM(response="[]", fail_times=2)
    assert extract_dates("hello", client=client, reference=TODAY, attempts=3) == []
...
def test_une_panne_persistante_est_retentee_trois_fois_pas_une_de_plus():
    ...
    with pytest.raises(ExtractionUnavailable):
        extract_dates("hello", client=client, reference=TODAY)
    assert client.call_count == 3
...
def test_production_l_adaptateur_parle_au_kit_par_chat_completions_create():
    ...
    assert extract_dates("réunion le 12/03/2024", client=ProviderClient(sdk=sdk), reference=TODAY) == [
        ("12/03/2024", date(2024, 3, 12))
    ]
    ...
    assert request["messages"] == [
        {"role": "user", "content": PROMPT.format(text="réunion le 12/03/2024", reference="2024-03-12")}
    ]
    assert request["temperature"] == 0
    assert len(sdk.requests) == 1
    other = FakeSDK(content="[]")
    extract_dates("hello", client=ProviderClient(sdk=other, model="another-model"), reference=TODAY)
...
def test_production_sans_client_le_kit_openai_est_construit_et_appele(monkeypatch):
    ...
    assert extract_dates("réunion le 12/03/2024", reference=TODAY) == [("12/03/2024", date(2024, 3, 12))]
...
def test_production_le_client_par_defaut_n_est_construit_qu_apres_les_refus_de_taille_et_de_texte_vide(monkeypatch):
    ...
    assert extract_dates("", reference=TODAY) == []
    assert extract_dates(" \n\t", reference=TODAY) == []
    with pytest.raises(ValueError):
        extract_dates("x" * (MAX_CHARACTERS + 1), reference=TODAY)
...
def test_production_un_texte_vide_ou_blanc_ne_coute_aucun_appel():
    for text in ("", "  \n "):
        client = FakeLLM(response="[]")
        assert extract_dates(text, client=client, reference=TODAY) == []
        assert client.call_count == 0


def test_le_plafond_leve_au_lieu_de_tronquer():
    """Docstring : « The cap on the input raises rather than truncating: a contract cut in half would come back with a list of deadlines that looks complete »."""
    client = FakeLLM(response=ANSWER)
    with pytest.raises(ValueError, match=f"longer than {MAX_CHARACTERS}"):
        extract_dates("x" * (MAX_CHARACTERS + 1), client=client, reference=TODAY)
    assert client.call_count == 0  # rien n'est envoyé, rien n'est tronqué


def test_production_exactement_8000_caracteres_passent_et_8001_sont_refuses():
    client = FakeLLM(response="[]")
    assert extract_dates("x" * MAX_CHARACTERS, client=client, reference=TODAY) == []
    with pytest.raises(ValueError):
        extract_dates("x" * (MAX_CHARACTERS + 1), client=client, reference=TODAY)
...
def test_production_le_plafond_compte_des_caracteres_pas_des_jetons():
    ...
    client = FakeLLM(response="[]")
    assert extract_dates("📅" * 5000, client=client, reference=TODAY) == []
    assert extract_dates("📅" * MAX_CHARACTERS, client=client, reference=TODAY) == []
    with pytest.raises(ValueError):
        extract_dates("📅" * (MAX_CHARACTERS + 1), client=client, reference=TODAY)
...
def test_une_reponse_enveloppee_dans_une_seule_cloture_est_decodee():
    """`_unfenced` : « Models often hand back a JSON answer inside one fenced block »."""
    for response in (f"```json\n{ANSWER}\n```", f"```\n{ANSWER}\n```"):
        client = FakeLLM(response=response)
        assert extract_dates("réunion le 12/03/2024", client=client, reference=TODAY) == [("12/03/2024", date(2024, 3, 12))]
        assert client.call_count == 1
...
def test_production_une_cloture_entouree_de_texte_double_ou_non_refermee_leve():
    for response in (
        ...
        with pytest.raises(ExtractionUnavailable):
            extract_dates("réunion le 12/03/2024", client=FakeLLM(response=response), reference=TODAY)
...
def test_production_un_passage_nul_absent_ou_non_textuel_devient_une_chaine_vide():
    ...
    assert extract_dates("réunion", client=client, reference=TODAY) == [
        ("", date(2024, 3, 12)),
        ("", date(2024, 3, 13)),
        ("", date(2024, 3, 14)),
    ]
...
def test_production_une_injection_dans_le_document_fait_passer_la_date_qu_elle_dicte():
    ...
    assert extract_dates(document, client=client, reference=TODAY) == [("échéance", date(2099, 1, 1))]
...
def test_production_accents_decomposes_et_espaces_insecables_partent_intacts():
    ...
    extract_dates(text, client=client, reference=TODAY)
...
def test_production_valeurs_aux_limites_du_calendrier_dans_la_reponse():
    ...
    assert extract_dates("x", client=client, reference=TODAY) == [
        ("a", date(2024, 2, 29)),
        ("d", date(9999, 12, 31)),
        ("f", date(24, 1, 1)),
    ]
...
def test_production_zero_essai_leve_sans_appel():
    client = FakeLLM(response="[]")
    with pytest.raises(ExtractionUnavailable):
        extract_dates("hello", client=client, reference=TODAY, attempts=0)
    assert client.call_count == 0
