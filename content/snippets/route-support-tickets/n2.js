...
export async function loadEncoder(name = MODEL_NAME) {
  ...
  // Full precision, as the Python encoder: the transformers.js README gives a
  // quantised model (q8) as the default on WASM.
...
export async function neighbours(index, ticket, k = 3) {
  ...
  if (index.vectors.length && vector.length !== index.vectors[0].length) {
    throw new RangeError('the index was encoded by another model: re-encode the archive');
  }
