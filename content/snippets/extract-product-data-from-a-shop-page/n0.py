...
asks for a dot: rather than read it as one thousand or as one, the price comes
back as None with the raw text beside it.
"""
...
def read_products(html) -> dict:
    ...
    for block in _blocks(html):
        try:
            nodes.extend(_flatten(json.loads(block)))
...
def _product(node) -> dict:
    offer = _flatten(node.get("offers"))
    ...
    return {
        ...
        "gtin": _text(node.get("gtin") or node.get("gtin13") or node.get("gtin8")),
        "brand": _text(brand.get("name") if isinstance(brand, dict) else brand),
        # None when the shop did not write a number, with the text it did write
        # beside it: a price read wrong is worse than a price not read.
        "price": float(price) if price and PRICE.match(price) else None,
        "price_text": price,
