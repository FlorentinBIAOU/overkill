...
def test_un_marqueur_hors_du_vocabulaire_du_modele_est_perdu_et_signale():
    """Commentaire de MARK : « a marker made of characters the vocabulary lacks is dropped »."""
    perfect = [("items selected", "éléments sélectionnés")]
    # Témoin : les pièces du marqueur retenu sont dans le vocabulaire du modèle
    # nommé, il les réécrit, et rien n'est signalé.
    ...
    sans_crochets = EchoTranslator(perfect, drop=MARKER_BRACKETS)
    result = translate("{count} items selected", model=sans_crochets)
    assert result["review"] is True
    assert result["warnings"] == ["variables differ from the source: expected {count}, got none"]
...
def test_le_modele_est_injecte_et_par_defaut_c_est_le_vrai():
    ...
    with pytest.raises(ModuleNotFoundError, match="transformers"):
        translate("Save")
    # Une source vide ou blanche est rendue telle quelle : rien n'est chargé
    # pour l'apprendre.
    assert translate("") == {"target": "", "review": False, "warnings": []}
    assert translate("   ")["target"] == "   "
...
def test_les_variables_icu_et_i18next_sont_protegees():
    """docstring : « An ICU plural or select message always goes to review » ; `{{count}}` est masquée entière."""
    icu = "{count, plural, one {# item} other {# items}}"
    translated = EchoTranslator([("count, plural, one", "compte, pluriel, un"), ("other", "autre")])
    result = translate(icu, model=translated)
    ...
    assert translated.calls == ["[0], one {# item} other {# items}}"]
    assert result["review"] is True
    assert result["warnings"] == ["ICU message: check its branches by hand"]
    # La variable d'i18next, elle, est masquée entière : elle revient intacte.
    i18next = FakeSeq2Seq({"[0] items": "[0] éléments"})
    assert translate("{{count}} items", model=i18next) == {
