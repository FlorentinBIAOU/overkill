...
def test_point_de_rupture_temoin_vingt_chaines_correctes_ressortent_inchangees():
    """
    « Le témoin : vingt chaînes déjà correctes, dont « Île-de-France » et
    « São Paulo », ressortent inchangées. »
    """
...
# ---------------------------------------------------------------------------
# Les deux langages, et leur seule divergence
# ---------------------------------------------------------------------------


def test_les_deux_extraits_saccordent_sauf_sur_la_regle_de_lespace():
    """
    Docstring : « The two agree on thirty-nine of this entry's forty-one
    strings ». Le décompte est fait ici.
    """
    node = shutil.which("node")
    assert node, "node est requis pour comparer les deux implémentations"
    entrees = (list(CASSES) + CORRECTES + [TICKET, A_TILDE_ESPACE, "Ã tout de suite",
    ...
    assert len(entrees) == 41
    # Les deux seules chaînes où ftfy applique sa règle de l'espace.
    assert ecarts == [A_TILDE_ESPACE, "Ã tout de suite"]
