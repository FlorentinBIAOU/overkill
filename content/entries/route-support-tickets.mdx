...
scenario:
  fr: "Le premier réflexe est un appel de modèle par ticket entrant, avec les noms des équipes dans l'invite. Pendant ce temps, l'outil de ticketing contient déjà tous les tickets résolus, chacun étiqueté par l'équipe qui l'a fermé : un jeu d'entraînement payé depuis des années et laissé de côté. Et la difficulté du routage n'est pas le ticket qui appartient clairement à une équipe, c'est celui qui appartient à deux, ou à aucune : aucun barreau ne le supprime, et chacun doit dire ce qu'il en fait."
  en: "The first move is usually one model call per inbound ticket, with the team names in the prompt. Meanwhile the ticketing system already holds every ticket the desk has resolved, each labelled with the team that closed it: a training set paid for years ago and left unused. And the hard part of routing is not the ticket that clearly belongs to one team, it is the one that belongs to two, or to none. No rung removes that ticket, and each has to say what it does with it."
