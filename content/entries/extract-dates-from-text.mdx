...
rungs:
  - level: N0
    ...
    breaking_point:
      ...
      en: "Relative dates. \"next Thursday\", \"in a fortnight\", \"from tomorrow\": there are no digits to match, so nothing at all is found. The failure is silent, an empty list rather than an error, and a planning tool built on this rung simply never sees half of what people write."
    escalate_when:
      fr: "Vos documents fixent les échéances par rapport à aujourd'hui, et vos extractions reviennent vides sur exactement ces phrases-là."
      en: "Your documents set deadlines relative to today, and your extraction comes back empty on exactly those sentences."
